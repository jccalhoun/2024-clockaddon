...
var showLeadingZero = false; // New preference for leading zero
...
browser.runtime.onMessage.addListener((message) => {
    ...
    if (message.colorChanged || message.formatChanged || message.displayChanged) { // Add check for display changes
        ...
        browser.storage.sync.get({
            ...
            showLeadingZero = storageResult.showLeadingZero;
...
function updateClock() {
    ...
    try {
        ...
        if (showLeadingZero && hours < 10) {
            textToDraw = '0' + hours;
        }

        console.log("Current hour text:", textToDraw);
        ...
        for (let currentSize = canvas.height; currentSize >= 1; currentSize--) {
            ...
            if (textWidth <= canvas.width - 1 && currentSize <= canvas.height - 1) {
                ...
                console.log(`Found best font size: ${bestFontSize}px for text "${textToDraw}"`);
